...
describe('that the categories view is working as expected', () => {
  ...
  it('should display a "filled" source on the first star-image after click', async () => {
    ...
    expect(starEmpty[0]).toHaveAttribute('src', '/Paketering-gruppexamination/src/assets/images/favourite-filled.png')
  })

  it('should display the movieview and the actors 2 first names', async () => {
    render(
      <Provider store={store}>
        <MemoryRouter initialEntries={['/Paketering-gruppexamination/categoriesview']}>
        <Routes>
        <Route
    ...
    expect(await screen.findByText('Christian Bale, Heath Ledger', {exact:false})).toBeInTheDocument()
  })

  it.only('should display the movieview and that the add to favorites button is rendered and that it is working', async () => {
    render(
      <Provider store={store}>
        <MemoryRouter initialEntries={['/Paketering-gruppexamination/categoriesview']}>
        <Routes>
    ...
    expect(await screen.findByText('Remove from favorites', {exact: false})).toBeInTheDocument();

    window.localStorage.clear()
  })

  it.only('should go from favorites>movieview>click-add>clicknav>favorites should be 1', async () => {
    render(
      <Provider store={store}>
        <MemoryRouter initialEntries={['/Paketering-gruppexamination/categoriesview']}>
        <Routes>
    ...
    const movieImage = await screen.findAllByAltText('movie thumbnail')
    await userEvent.click(movieImage[0])
    const button = await screen.findByText('Add to favorites', {exact: false})
    await userEvent.click(button)
