...
function Recommended() {
  ...
  const initSlider = () => {
    const slideButtons = Array.from(document.getElementsByClassName("slide-button")) as HTMLButtonElement[]
        ...
        <div className='slider-wrapper'>
          ...
          <div className='image-list' ref={imageListRef}>
            {isNotTrendingComp.map((movie: MovieData, index: number) => {
              return (
                <img className='image-item' key={index} src={movie.thumbnail} alt={`Movie ${index}`} />
              )
            })}
          </div>
